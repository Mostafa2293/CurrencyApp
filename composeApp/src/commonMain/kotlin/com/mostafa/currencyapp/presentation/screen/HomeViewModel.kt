...
class HomeViewModel(
    ...
    fun sendEvent(event: HomeUiEvents) {
        when (event) {
            is HomeUiEvents.RefreshRates -> {
                screenModelScope.launch {
                    fetchNewRates()
                }
            }
            is HomeUiEvents.SwitchCurrencies -> {
                switchCurrencies()
            }
            is HomeUiEvents.SaveSourceCurrencyCode -> {
                saveSourceCurrencyCode(event.code)
            }
            is HomeUiEvents.SaveTargetCurrencyCode -> {
